...
const FoodDeliveryCreateOrder = (currentStep) => {
  ...
  useEffect(() => {
    if (mealList.length > 0 && !initialSelectionDone) {
      const storedItems = localStorage.getItem('selectedItems');
      if (storedItems) {
        const parsedItems = JSON.parse(storedItems);
        console.log('Stored items from localStorage:', parsedItems);

        let totalPrice = 0;
        const newSelectedDishes = [];
        const newSelectedDishDictionary = {};

        mealList.forEach(meal => {
          meal.dish.forEach(dish => {
            parsedItems.forEach(storedItem => {
              if (dish.name.toLowerCase() === storedItem.title.toLowerCase()) {
                console.log('Found matching dish:', dish.name);
                
                newSelectedDishes.push(dish._id);
                newSelectedDishDictionary[dish._id] = dish;
                
                const dishPrice = parseInt(dish.cuisineArray[0], 10);
                if (!isNaN(dishPrice)) {
        ...
        if (newSelectedDishes.length > 0) {
          ...
          setSelectedCount(newSelectedDishes.length);
          setSelectedDishPrice(totalPrice);
          setSelectedDishDictionary(newSelectedDishDictionary);
          setIsDishSelected(true);
          setInitialSelectionDone(true);
          
          console.log('Pre-selected dishes:', newSelectedDishes);
  ...
  const handleIncreaseQuantity = (dish, isSelected) => {
    console.log(dish, isSelected, "both");
    if (selectedDishes.length >= 15 && !isSelected) {
      ...
      console.log(updatedSelectedDishes,"updatee");
...
console.log('selectedOption',selectedOption)
  ...
  const addDish = selectedDishPrice => {
    console.log(selectedDishPrice,"sleectedishprice");
    if (!selectedDishDictionary || Object.keys(selectedDishDictionary).length === 0) {
      ...
      console.log(selectedDishQuantities,"newone");
